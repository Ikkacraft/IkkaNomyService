...
@Api(value = "/accounts", description = "Operations about accounts", produces="application/json, application/xml")
class Accounts @Inject()(accountService: AccountService) extends Controller {
  ...
  def getAccounts() = Action { request =>
    request.accepts("application/json") || request.accepts("text/json") match {
      case true =>{
        ...
        if (accounts.isEmpty) NotFound("Users not found") else Ok(Json.toJson(accounts))
      }
      case false => {
        val accounts: List[Account] = accountService.getAll()
        if (accounts.isEmpty) NotFound("Users not found") else Ok(<accounts>{accountService.getAll().map(a => a.toXml)}</accounts>)
      }
    }
  }
  ...
  def getAccount(account_id: Long) = Authorized { request =>
    request.accepts("application/json") || request.accepts("text/json") match {
      case true => {
  ...
  @ApiOperation(
    nickname = "getTransactionsByAccount",
    value = "Get all transactions for an account",
    notes = "Return a list of transaction",
    response = classOf[models.Account], httpMethod = "GET")
  ...
  def getTransactionsByAccount(account_id: Long) = Authorized { request =>
    request.accepts("application/json") || request.accepts("text/json") match {
      case true => {
  ...
  def createAccount() = Action(parse.json) { implicit request =>
    ...
    id match {
      ...
      case _  => Created("Account " + id + " has been successfully created")
    }
  }
  ...
  def updateAccount(account_id: Long) = Authorized(parse.json) { implicit request =>
  ...
  response = classOf[models.Account], httpMethod = "DELETE")
  @ApiResponses(Array(
  new ApiResponse(code = 200, message = "The account has been successfully deleted")
  ))
  def deleteAccount(account_id: Long) = Authorized {
